  ...
  return (
    <>
      ...
      <Content
        style={contentStyle}
      >
        <Tabs defaultActiveKey="1" onChange={(key) => dispatch(actions.activeOrderTab(key))}>
          <TabPane tab={`Незаконченные (${orders.pending.list.length})`} key="4">
            <OrdersTable orders={orders.pending.list} loading={orders.loading} />
          </TabPane>
