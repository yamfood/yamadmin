...
const ProductModifiers = () => {
  ...
  const [modifiersStateFilter, setModifiersStateFilter] = useState(() => (modifier) => modifier);
    ...
    if (target.value) {
      setNameFilter(
        () => (modifier) => modifier.name.toLowerCase().includes(target.value.toLowerCase()) ||
          modifier.products.some((product) => product?.name?.toLowerCase().includes(target.value.toLowerCase())),
      );
      return
    }
  ...
  useEffect(() => {
    setModifiers(list.filter(nameFilter).filter(modifiersStateFilter))
  }, [list, nameFilter, modifiersStateFilter]);
  ...
  const expandedRowRender = (modifier) => {
    ...
    return <Table key={modifier.id + '_table'}
