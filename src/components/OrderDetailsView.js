...
const OrderDetailsView = (props) => {
  ...
  const columns = [
      ...
      render: (value, product) => (
        <>
          {form.getFieldDecorator(
            `products.${product.id}[${product.index}].comment`,
            {initialValue: value},
          )(<Input disabled={!allowLiteEdit}/>)}
          {form.getFieldDecorator(
            `products.${product.id}[${product.index}].payload`,
            {initialValue: product.payload ? product.payload : {}}
          )(<Input type="hidden"/>)}
          {form.getFieldDecorator(
            `products.${product.id}[${product.index}].product_id`,
    ...
    },
    {
      ...
      key: 'count',
      width: '100px',
      render: (value, product) => form.getFieldDecorator(
    ...
    },
    {
      ...
      render: (total, product, index) => (calculateProductPrice(product, index) || 0)
        * (form.getFieldValue(`products.${product.id}[${product.index}].count`) || 1),
    },
  ];
          ...
          expandedRowKeys={
            order.products.filter(
              (product) => (availableProducts
                && Object.keys(availableProducts[product.id]?.modifiers || {})?.length > 0),
            ).map((product) => `${product.id}_${product.index}`)
          }
